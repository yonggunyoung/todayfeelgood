...
## 8. 작업 방식 5원칙 (항상 적용 — 마스터 지시)
> 모든 작업에 기본 적용한다. 충돌 시 이 원칙을 우선해 신중함을 택한다.
1. **불신을 기본값으로 (계약적 사고).** 외부 입력·모델 출력·과거/미래의 자기 자신을 일단 의심한다.
   값은 재조립(예: 모델 날짜 버리고 엔진 날짜)·clamp(예: 점수 1~5)하고, **경계 4종(정상/매핑/None/변조)** 테스트로 두른다.
2. **결정을 추적 가능한 객체로 외화한다.** 결정엔 ID(D1·D2…)를 붙이고, 코드 주석/문서에 **이유+비용+탈출구**를 남긴다. 머릿속·채팅에 두지 않는다.
3. **풀기 전에 경계부터 선언한다.** "건드리지 않는 것"을 먼저 적어 변경 반경을 좁힌다 → 스코프를 발산이 아니라 **수렴**으로 연다.
4. **가역성을 선호한다.** 갈아엎지 않고 기존 위에 얹는다. 변경 이유는 별도 커밋으로 보존한다.
5. **'완료'의 정의가 넓다.** 코드가 도는 것 = 끝이 아니다. **동작 + 테스트 + 문서 현행화 + 검증**까지가 한 단위.
