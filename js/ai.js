...
const PROMPT = `이 사진은 한국 마트/편의점/온라인몰 영수증 또는 장 봐온 식품 사진이에요. 기울어지거나 구겨졌어도 최대한 읽어주세요.
...
[정규화] 브랜드·매장·용량·바코드 접두/접미를 떼고 핵심 품목명만: "CJ나트룸줄인국산찹쌀고추장 1kg"→고추장, "오리온)비쵸비 5P"→비쵸비, "파)로만밀식빵 420g"→식빵, "카스/올제로 500ml"→맥주, "상추 1봉"→상추. '1+1·×3'은 수량으로, 없으면 1.
[빈 목록 금지] 애매해도 식품으로 보이면 반드시 포함하되 confidence만 낮게(0.3~0.6). 사람이 읽을 품목 줄이 하나라도 있으면 최선의 추측이라도 넣으세요.
