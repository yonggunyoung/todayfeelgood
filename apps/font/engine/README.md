# Font Engine (Phase 2 · 전통/비AI 방식) — 계약 v3
...
### `POST /generate`
요청 (`GenerateRequest`, `packages/core` 계약 v3):
```json
{
  "params": {
...
```json
{
  ...
  "appliedParams": { "weight": 600, "slant": -6, "curvature": 0.5, "mono": 0.8, "cursive": 1, "weirdness": 40, "seed": 7, "letterSpacing": 0.05 }
}
```
...
pytest -q
```
- `test_generator.py`: clamp(기존+신규 범위/NaN/Inf), woff/ttf 매직, weight 매핑(가는↔굵은 차이), weirdness 재현성(같은 seed→동일·다른 seed→상이·0=정형), letterSpacing advance 증가, 한글 woff/ttf·한글 글리프 cmap 포함·서브셋 크기 제한.
- `test_api.py`: v3 응답 형태(`script`/`generatedBy:baseFontVariation`), latin/hangul × woff/ttf, 잘못된 format/script 422, 범위 밖(기존+신규) 422, imagePng 초과 413, /health(`hangul_font_loaded`).
