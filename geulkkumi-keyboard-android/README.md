...
### ⭐ 자주쓰기 (핵심) — “채팅창에서 저장한 것 바로 꺼내쓰기”
- 키보드 첫 탭이 **⭐ 자주쓰기**: 멋글씨 행의 **☆ 를 누르면 저장**, 한 번 입력한 건 **🕑 최근**에 자동 누적.
- 어느 앱의 입력창에서든 저장/최근 항목을 **한 번 탭 → 그 자리에 즉시 입력**. (PWA가 다른 앱 위에 못 그리는 한계를 키보드가 해결)
- 저장소는 **키보드 자체 WebView localStorage**(`geulkkumi.kb.v1`) — 브라우저로 쓰는 글꾸미 앱과는 분리된 영역이라, 자주쓰기는 키보드에서 따로 쌓입니다.
...
- ~~즐겨찾기/최근~~ → **구현됨**(⭐ 자주쓰기 탭, WebView localStorage). 필요 시 메인 앱과 동기화하려면 일회성 ‘내보내기 코드’ 방식 등 별도 설계 필요(저장소 샌드박스가 분리되어 자동 공유는 불가).
