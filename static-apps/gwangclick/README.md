...
## 👥 초반 가상 플레이어 (`gc-bots.js`)
출시 초반 실제 접속자가 붙기 전 "전국 집계 0 → 죽은 앱"으로 보이는 걸 막는 **콜드스타트 보정**.
- **표시 전용**: 게이지·참전수·전국 랭킹·지역 점령·지구본에만 합성. **DB(Firestore)엔 절대 기록 안 함 → 실데이터 무오염.**
- **결정적(seed=날짜)**: 모든 기기가 같은 수치를 봄. 시간대 곡선으로 낮엔 적고 저녁에 많아짐(현재 새벽 ~200 / 저녁 ~750명 규모).
- **자동 소멸**: 실제 참여자가 `FADE`(기본 350명)에 가까워질수록 가상치가 0으로 페이드 → 손 안 대도 자연 전환.
- **킬 스위치**: 충분히 성장하면 `gc-bots.js`의 `enabled=false` 한 줄로 완전 OFF. 규모 조절은 `FLOOR`/`PEAK` 상수.
