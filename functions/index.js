...
const SCAN_PROMPT = `이 사진은 한국 마트/편의점/온라인몰 영수증 또는 장 봐온 식품 사진이에요. 기울어지거나 구겨졌어도 최대한 읽어주세요.
[추출] 먹거나 주방·냉장고에 보관하는 모든 식품 — 채소·과일·육류·수산·계란·유제품·두부·면·쌀·빵·과자·음료·주류·양념/소스·통조림·냉동/즉석식품·반찬·간식까지 전부.
[제외] 명백한 비식품(세제·휴지·위생용품·주방잡화·봉투값)과 요약/결제 줄(할인·면세·과세·합계·부가세·포인트·카드·승인·잔액)만.
[정규화] 브랜드·매장·용량·바코드 접두/접미를 떼고 핵심 품목명만: "CJ나트룸줄인국산찹쌀고추장 1kg"→고추장, "오리온)비쵸비 5P"→비쵸비, "파)로만밀식빵 420g"→식빵, "파)정통크림빵3입"→크림빵, "카스/올제로 500ml"→맥주, "상추 1봉"→상추, "양념고추장등갈비"→등갈비. '1+1·2입·×3'은 수량으로, 없으면 1.
[빈 목록 금지] 애매해도 식품으로 보이면 반드시 포함하되 confidence만 낮게 주세요. 사람이 읽을 품목 줄이 하나라도 있으면 최선의 추측이라도 넣고 절대 items를 비우지 마세요.`;
