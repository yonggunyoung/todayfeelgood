...
## 6) (선택) 구글 로그인 켜기 — '이름 알리기'용
설정 화면의 "구글로 로그인"이 동작하려면 콘솔에서 1회만:
1. Authentication → **로그인 방법** → **Google** → 사용 설정(지원 이메일 선택) → 저장
2. Authentication → 설정 → **승인된 도메인**에 `ddukkit.com` 있는지 확인(없으면 추가)
→ 끝. 익명으로 놀던 사람이 로그인하면 기존 기록이 그대로 그 계정으로 연결되고, 다른 기기에서 같은 구글로 로그인하면 동일 신원으로 이어집니다. (설정 안 하면 로그인 버튼이 '실패' 토스트만 뜨고 게임엔 영향 없음)
