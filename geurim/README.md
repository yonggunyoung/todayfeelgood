...
| Gemini Advanced 구독 | **Gemini API 키** (무료 한도 + 사용량 과금) | [aistudio.google.com](https://aistudio.google.com/apikey) → Get API key |

> ChatGPT/Gemini 앱 구독료로는 API가 호출되지 않습니다. 위 콘솔에서 키를 발급하고 결제수단을 등록해야 이미지 생성이 동작합니다. 비용은 만든 장수만큼 **본인 계정에 과금**됩니다.
...
- **공용 PC에서는 키를 넣지 마세요.** 설정 → "설정·키 초기화" 로 언제든 지울 수 있습니다.
...
geurim/
  ...
  js/
    ...
    providers/
      index.js            모델 카탈로그 + 직접/프록시 베이스 해석 + 디스패치
      openai.js           OpenAI 이미지·텍스트 (순수 빌더 + fetch)
      gemini.js           Gemini 이미지·텍스트 (순수 빌더 + fetch)
